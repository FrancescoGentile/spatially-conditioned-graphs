...
class HOINetworkTransform(transform.GeneralizedRCNNTransform):
    ...
    def resize(self, image, target):
        ...
        scale_factor = min(self.min_size[0] / min_size, self.max_size / max_size)
        ...
        target["boxes_h"] = transform.resize_boxes(
            target["boxes_h"], (h, w), image.shape[-2:]
        )
        target["boxes_o"] = transform.resize_boxes(
            target["boxes_o"], (h, w), image.shape[-2:]
        )
