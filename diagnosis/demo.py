...
def colour_pool(n):
    pool = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "#17becf", "#e377c2"]
    nc = len(pool)
...
def draw_boxes(ax, boxes):
    ...
    for i, (a, b, c) in enumerate(zip(xy, h.tolist(), w.tolist())):
        ...
        ax.add_patch(patch)
        txt = plt.text(
            *a.tolist(), str(i), fontsize=20, fontweight="semibold", color="w"
        )
...
def main(args):
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset, collate_fn=custom_collate, batch_size=4, shuffle=False
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--detection-dir",
        default="../hicodet/detections/test2015",
        type=str,
        help="Directory where detection files are stored",
    )
