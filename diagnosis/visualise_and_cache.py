...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dir", help="Directory where .mat files are stored", type=str, required=True
    )
    parser.add_argument(
        "--data-root",
    ...
    parser.add_argument(
        "--debug", help="Only generate results for one class", action="store_true"
    )
    args = parser.parse_args()

    dataset = pocket.data.HICODet(
        os.path.join(
            args.data_root, "hico_20160224_det/images/{}".format(args.partition)
        ),
    ...
    for f in fnames:
        ...
        for intra_idx, boxes in enumerate(all_boxes):
            ...
            for j, k in zip(keep, sorted_idx):
                image = Image.open(
                    os.path.join(dataset._root, dataset._filenames[k[0]])
                )
                target = dataset._anno[k[0]]

                b1 = boxes[k[0]][k[1], :4]
                b2 = boxes[k[0]][k[1], 4:8]
                ...
                canvas.line(
                    b_h_centre.tolist() + b_o_centre.tolist(), fill="#FF4444", width=5
                )
                canvas.ellipse(
                ...
                gt_idx = torch.nonzero(
                    target["hoi"] == torch.as_tensor(hoi_idx)
                ).squeeze(1)
                if len(gt_idx):
                    ...
                    pocket.utils.draw_dashed_rectangle(
                        image, gt_b1[idx].tolist(), fill="#007CFF", width=5
                    )
                    pocket.utils.draw_dashed_rectangle(
                        image, gt_b2[idx].tolist(), fill="#46FF00", width=5
                    )
                ...
                cache_name = "{}_{}_{}_{}_{:.4f}.png".format(
                    j, *k, int(sorted_labels[j]), scores[order][j]
                )
