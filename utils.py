...
class DataFactory(Dataset):
    def __init__(
        ...
        if name == "hicodet":
            ...
            self.dataset = HICODet(
                ...
                anno_file=os.path.join(
                    data_root, "instances_{}.json".format(partition)
                ),
            ...
            self.dataset = VCOCO(
                ...
                anno_file=os.path.join(
                    data_root, "instances_vcoco_{}.json".format(partition)
                ),
                target_transform=pocket.ops.ToTensor(input_format="dict"),
            )
            self.human_idx = 1
        ...
        self._flip = (
            torch.randint(0, 2, (len(self.dataset),))
            if flip
    ...
    def __getitem__(self, i):
        ...
        detection_path = os.path.join(
            self.detection_root, self.dataset.filename(i).replace("jpg", "json")
        )
...
def test(net, test_loader):
    ...
    meter = DetectionAPMeter(
        600, nproc=1, num_gt=testset.anno_interaction, algorithm="11P"
    )
    net.eval()
    for batch in tqdm(test_loader):
        ...
        interactions = torch.tensor(
            [testset.object_n_verb_to_interaction[o][v] for o, v in zip(objects, verbs)]
        )
...
class CustomisedDLE(DistributedLearningEngine):
    ...
    def _on_end_epoch(self):
        ...
        if self._rank == 0:
            print(
                "Epoch: {} | training mAP: {:.4f}, evaluation time: {:.2f}s |"
    ...
    def _print_statistics(self):
        ...
        if self._rank == 0:
            print(
                f"=> HOI classification loss: {hoi_loss:.4f},",
    ...
    def _synchronise_and_log_results(self, output, meter):
        ...
        all_results = np.stack(
            [np.concatenate(scores), np.concatenate(pred), np.concatenate(labels)]
        )
