...
def main(rank, args):
    ...
    val_loader = DataLoader(
        ...
        num_workers=args.num_workers,
        pin_memory=True,
        sampler=DistributedSampler(valset, num_replicas=args.world_size, rank=rank),
    )
    ...
    if os.path.exists(args.checkpoint_path):
        print(
            "=> Rank {}: continue from saved checkpoint".format(rank),
    ...
    def lambda1(epoch):
        return 1.0 if epoch < args.milestones[0] else args.lr_decay
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train-detection-dir", default="hicodet/detections/train2015", type=str
    )
    parser.add_argument(
        "--val-detection-dir", default="hicodet/detections/test2015", type=str
    )
    parser.add_argument(
        "--num-iter",
        default=2,
    ...
    parser.add_argument(
        "--batch-size", default=4, type=int, help="Batch size for each subprocess"
    )
    parser.add_argument(
        "--lr-decay",
        default=0.1,
